// Package simhash  implements Charikar's simhash algorithm to generate a 64-bit
...
// Vector is a 64-dim vector
type Vector [64]int

// Feature consists of a 64-bit hash and a weight
type Feature interface {
	// Sum returns the 64-bit sum of this FeatureT
	Sum() uint64

	// Weight returns the weight of this FeatureT
	Weight() int
}
...
// incremented by weight of the i-th FeatureT if the i-th bit of the FeatureT
// is set, and decremented by the weight of the i-th FeatureT otherwise.
...
// where each []byte is a FeatureT with even weight.
//
// Vectors are initialized to zero. The i-th element of the vector is then
// incremented by weight of the i-th FeatureT if the i-th bit of the FeatureT
// is set, and decremented by the weight of the i-th FeatureT otherwise.
...
// Sum returns the 64-bit hash of this FeatureT
func (f FeatureT) Sum() uint64 {
	return f.sum
}

// Weight returns the weight of this FeatureT
func (f FeatureT) Weight() int {
	return f.weight
}

// NewFeature returns a new FeatureT representing the given byte slice, using a weight of 1
func NewFeature(f []byte) FeatureT {
	h := fnv.New64()
	_, err := h.Write(f)
	if err != nil {
		log.Panic(err)
	}
	return FeatureT{h.Sum64(), 1}
}

// NewFeatureWithWeight returns a new FeatureT representing the given byte slice with the given weight
func NewFeatureWithWeight(f []byte, weight int) FeatureT {
	fw := NewFeature(f)
	fw.weight = weight
	return fw
}
...
// Simhash returns a 64-bit simhash of the given FeatureT set
func Simhash(fs FeatureSet) uint64 {
	return Fingerprint(Vectorize(fs.GetFeatures()))
}

// WordFeatureSet is a FeatureT set in which each word is a FeatureT,
// all equal weight.
type WordFeatureSet struct {
	b []byte
}

// NewWordFeaureSet constructs a FeatureT set from b
...
// GetFEatures returns a []Feature representing each word in the byte slice
func (w *WordFeatureSet) GetFeatures() []Feature {
	return getFeatures(w.b, boundaries)
}

// UnicodeWordFeatureSet is a FeatureT set in which each word is a FeatureT,
// all equal weight.
//
...
// NewUnicodeWordFeatureSet returns FeatureT set (Unicode format)
